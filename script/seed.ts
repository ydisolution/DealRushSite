...
async function seed() {
  ...
  try {
    ...
    // Create 38 additional customers (total 40)
    console.log("👥 Creating 38 additional customers...");
    const customers = [nir, shlomi];
    const firstNames = ["אורי", "דנה", "יוסי", "מיכל", "רועי", "שירה", "עומר", "נועה", "איתי", "תמר",
                       ...
                       "ארז", "סיגל", "רן", "ורד", "טל", "אסף", "מיכאל", "לי"];
    const lastNames = ["כהן", "לוי", "מזרחי", "פרץ", "ביטון", "דהן", "אבוטבול", "עזרא"];
    ...
    console.log(`✅ Created 40 total customers`);
    ...
    // Add varied participants to active deals (distributed across tiers)
    console.log("🛒 Adding participants to active deals with tier distribution...");
    
    // Distribution: 8 deals in tier 1, 7 in tier 2, 5 in tier 3
    const tierDistribution = [1, 2, 0, 4, 3, 6, 1, 7, 2, 5, 0, 4, 3, 8, 1, 2, 6, 0, 3, 4];
    
    for (let i = 0; i < Math.min(activeDeals.length, tierDistribution.length); i++) {
      const [activeDeal] = activeDeals[i];
      const numParticipants = tierDistribution[i];
      ...
      if (numParticipants > 0) {
        for (let j = 0; j < numParticipants; j++) {
          ...
          let tierIndex = 0;
          let pricePaid = activeDeal.tiers[0].price;
          for (let t = activeDeal.tiers.length - 1; t >= 0; t--) {
            if (position >= activeDeal.tiers[t].minParticipants + 1) {
              tierIndex = t;
              pricePaid = activeDeal.tiers[t].price;
              break;
            }
          }
        ...
        await db.update(deals).set({ participants: numParticipants }).where(eq(deals.id, activeDeal.id));
      }
    }
    
    console.log(`✅ Added participants to ${tierDistribution.filter(n => n > 0).length} active deals`);
